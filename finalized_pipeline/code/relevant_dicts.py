"""
Modifier word lists, alternate-form groupings, and per-word explanations for the
degree-modifier classification pipeline (EN + JP).

Alterations relative to the original prompt_explanation_store.py:
  - EN_MODIFIER_EXPLANATIONS["only"/"further"/"just about"]: non-examples added.
  - EN_MODIFIER_EXPLANATIONS["indefinitely"]: added (was missing).
  - EN_MODIFIER_EXPLANATIONS["especially"] / JP_MODIFIER_EXPLANATIONS["とくに・特に"]:
    clarified as scalar focus particles that are ALWAYS degree modifiers, whether they
    precede a bare adjective or a clause/topic noun phrase -- they do not exclude the
    other, non-highlighted cases the way true restrictive words (only/だけ) do.
  - JP_MODIFIER_EXPLANATIONS["ほとんど・殆ど"]: ほとんど+negation reclassified as a
    degree-modifier example, matching EN "hardly"/"barely".
  - JP_MODIFIER_EXPLANATIONS["どうも"]: どうも+ありがとう/すみません reclassified as a
    degree-modifier example, matching EN "thank you very much".
"""
...
JP_MODIFIER_EXPLANATIONS = {
    ...
    'ほとんど・殆ど': 'Explanation: The word ほとんど can function as a modifier of degree ("mostly/almost entirely"), marking a very high or (under negation) very low degree -- similar to English "almost," "hardly," or "barely." It has no major competing non-degree function when it scales a gradable predicate:\n- Example: ほとんど終わった（＝大部分、程度を表す）\n- Example: ほとんど食べない（＝「ほとんど＋否定」で、英語の "hardly eats/barely eats" と同様に、行為がほぼゼロに近い程度で行われることを表す程度表現）\n- Non-example: ほとんどの人がそう思う／そのほとんどが宣伝行為だ（＝「ほとんどの／そのほとんどが」の形で名詞・数量を修飾する準体的な用法で、程度を表す副詞ではなく「大部分の」という数量限定表現）',
    ...
    'とくに・特に': 'Explanation: The word とくに is a scalar focus particle functioning as a modifier of degree/emphasis ("especially/particularly"), highlighting a case that exceeds the general degree. This holds whether it directly precedes a gradable predicate/adjective or precedes a noun phrase naming a specific topic or example -- it never excludes the other, non-highlighted cases, it only marks the highlighted one as more salient/extreme, so it is not restrictive/exclusive. It has no major competing non-degree function:\n- Example: 特に難しい問題だ（＝とりわけ、程度の際立ちを示す、形容詞を直接修飾）\n- Example: この問題は特にJRAにおいて深刻だ（＝他の団体でも問題がないわけではなく、JRAという事例が際立って程度が高いことを示す、名詞句の前でも程度表現）\n- Example: 特に著作権に関わる問題では、慎重になるべきだ（＝他の場合を排除するのではなく、この場合の程度・重要性が際立つことを示す）',
    ...
    'どうも': 'Explanation: The word どうも functions as a modifier of degree ("very much/quite"), intensifying the sincerity or strength of the expression it precedes -- including in the conventionalized gratitude/apology construction どうも＋ありがとう／すみません／失礼しました, which is the direct equivalent of English "thank you very much" (also treated as a degree modifier). It has no major competing non-degree function:\n- Example: どうも様子がおかしい（＝なんとなく、程度の曖昧な強調）\n- Example: どうも、ありがとうございます（＝「どうも」が「ありがとう」を強めており、英語の "thank you very much" の "very much" と同じ機能）\n- Example: どうもすみませんでした（＝謝罪の程度を強める用法）',
...
EN_MODIFIER_EXPLANATIONS = {
    ...
    'just about': 'Explanation: These words can all function as modifiers of degree ("very close to, but not quite"), marking that a state or event is nearly but not fully reached. "Practically" additionally has a competing manner-adverb function:\n- Example: The tank was almost empty. (= very close to; scales the gradable state "empty")\n- Example: She was nearly finished with the report. (= very close to; scales the gradable state "finished")\n- Non-example: He handled the crisis practically, without panicking. (= in a practical/pragmatic manner; a manner adverb describing how he acted, not a degree modifier)\n- Non-example: You can edit just about any article on Wikipedia. (= almost/nearly the entire scope of "any article"; approximates a quantity/scope, not the intensity of a gradable adjective or verb)',
    ...
    'especially': 'Explanation: The word "especially" is a scalar focus particle functioning as a modifier of degree ("to an unusually high extent"), marking that a case exceeds the general degree. This holds whether it precedes a single adjective or a longer clause/noun phrase -- it never excludes the other cases, it only marks the highlighted one as more extreme, so it is not restrictive/exclusive. It has no major competing non-degree function:\n- Example: The exam was especially difficult this year. (= particularly, to a heightened degree)\n- Example: You may be blocked, especially if you violate the three-revert rule. (= the general risk is heightened in this case; blocking does not ONLY happen for 3RR violations, so this is degree-scaling, not restrictive)\n- Example: This is especially true regarding biographies of living persons. (= heightened applicability to this topic, not an exclusive restriction to it)',
    ...
    'only': 'Explanation: The word "only" can function as a modifier of degree ("merely, to a limited extent"), downplaying scope or significance. But it also very commonly has a RESTRICTIVE/EXCLUSIVE function, narrowing a set or scope ("solely, exclusively") rather than scaling the intensity of a gradable adjective or verb -- this restrictive use is NOT a degree modifier:\n- Example: It was only a minor issue. (= merely, to a limited extent; scales down "minor")\n- Example: It\'s only slightly late. (= merely; scales down "slightly late")\n- Non-example: I only fixed the typo. (= solely/exclusively that action, restricting what was done -- not scaling the degree of "fixed")\n- Non-example: Only three editors voted. (= exclusively/no more than; restricts a set/count, not a degree modifier)',
    ...
    'indefinitely': 'Explanation: The word "indefinitely" can function as a modifier of degree/extent ("for an unlimited or unspecified amount of time/to an unbounded extent"). It has no major competing non-degree function:\n- Example: The discussion was postponed indefinitely. (= for an unbounded, unspecified extent of time)',
}

EN_ALL_MODIFIER_WORDS = [
    ('\\bsorta\\b', 'sorta'),
    ('\\bin\\ any\\ way\\b', 'in any way'),
    ('\\bintensely\\b', 'intensely'),
    ('\\bperfectly\\b', 'perfectly'),
    ('\\bkinda\\b', 'kinda'),
    ('\\bplain\\b', 'plain'),
    ('\\bdamn\\b', 'damn'),
    ('\\bindefinitely\\b', 'indefinitely'),
    ('\\bsomewhat\\b', 'somewhat'),
    ('\\breally\\b', 'really'),
    ('\\bsupremely\\b', 'supremely'),
    ('\\bfricking\\b', 'fricking'),
    ('\\bconsiderably\\b', 'considerably'),
    ('\\bhella\\b', 'hella'),
    ('\\bexclusively\\b', 'exclusively'),
    ('\\bmost\\b', 'most'),
    ('\\bdead\\b', 'dead'),
    ('\\bmildly\\b', 'mildly'),
    ('\\bimmensely\\b', 'immensely'),
    ('\\bgoddamn\\b', 'goddamn'),
    ('\\bthoroughly\\b', 'thoroughly'),
    ('\\bphenomenally\\b', 'phenomenally'),
    ('\\bproportionally\\b', 'proportionally'),
    ('\\bterribly\\b', 'terribly'),
    ('\\bsick\\b', 'sick'),
    ('\\bso\\b', 'so'),
    ('\\bquite\\b', 'quite'),
    ('\\btotally\\b', 'totally'),
    ('\\bblindingly\\b', 'blindingly'),
    ('\\bastoundingly\\b', 'astoundingly'),
    ('\\bclosely\\b', 'closely'),
    ('\\bvery\\b', 'very'),
    ('\\ba\\ tad\\b', 'a tad'),
    ('\\bachingly\\b', 'achingly'),
    ('\\bkind\\ of\\b', 'kind of'),
    ('\\bproper\\b', 'proper'),
    ('\\bcompletely\\b', 'completely'),
    ('\\bpositively\\b', 'positively'),
    ('\\benough\\b', 'enough'),
    ('\\bstaggeringly\\b', 'staggeringly'),
    ('\\bstrongly\\b', 'strongly'),
    ('\\bspectacularly\\b', 'spectacularly'),
    ('\\bhardly\\b', 'hardly'),
    ...
    ('\\btoo\\b', 'too'),
    ('\\bunduly\\b', 'unduly'),
    ('\\bstrictly\\b', 'strictly'),
    ('\\ba\\ little\\ bit\\b', 'a little bit'),
    ('\\bvirtually\\b', 'virtually'),
    ('\\bmonumentally\\b', 'monumentally'),
    ('\\bsufficiently\\b', 'sufficiently'),
    ('\\boverly\\b', 'overly'),
    ('\\bappreciably\\b', 'appreciably'),
    ('\\bentirely\\b', 'entirely'),
    ('\\bultra\\b', 'ultra'),
    ('\\bnotably\\b', 'notably'),
    ('\\bfreaking\\b', 'freaking'),
    ('\\bdirectly\\b', 'directly'),
    ('\\bsignificantly\\b', 'significantly'),
    ('\\bwicked\\b', 'wicked'),
    ('\\bmore\\b', 'more'),
    ('\\bseriously\\b', 'seriously'),
    ('\\bexceptionally\\b', 'exceptionally'),
    ('\\bincredibly\\b', 'incredibly'),
    ('\\bvaguely\\b', 'vaguely'),
    ('\\bgreatly\\b', 'greatly'),
    ('\\bbreathtakingly\\b', 'breathtakingly'),
    ('\\bslightly\\b', 'slightly'),
    ('\\bfaintly\\b', 'faintly'),
    ('\\bminimally\\b', 'minimally'),
    ('\\binsanely\\b', 'insanely'),
    ('\\binfinitely\\b', 'infinitely'),
    ('\\btremendously\\b', 'tremendously'),
    ('\\bin\\ full\\b', 'in full'),
    ('\\bway\\b', 'way'),
    ("\\blil'\\b", "lil'"),
    ('\\bunusually\\b', 'unusually'),
    ('\\butterly\\b', 'utterly'),
    ('\\bmerely\\b', 'merely'),
    ('\\babsolutely\\b', 'absolutely'),
    ('\\bcrazy\\b', 'crazy'),
    ('\\bessentially\\b', 'essentially'),
    ('\\beffing\\b', 'effing'),
    ('\\bnearly\\b', 'nearly'),
    ('\\bat\\ all\\b', 'at all'),
    ('\\bmega\\b', 'mega'),
    ('\\bmuch\\b', 'much'),
    ('\\bscarcely\\b', 'scarcely'),
    ('\\bgenuinely\\b', 'genuinely'),
    ('\\badequately\\b', 'adequately'),
    ('\\bespecially\\b', 'especially'),
    ('\\bstupid\\b', 'stupid'),
    ('\\ba\\ hair\\b', 'a hair'),
    ('\\bpoorly\\b', 'poorly'),
    ('\\bexcessively\\b', 'excessively'),
    ('\\bnominally\\b', 'nominally'),
    ('\\bthe\\ most\\b', 'the most'),
    ('\\bextraordinarily\\b', 'extraordinarily'),
    ('\\bjolly\\b', 'jolly'),
    ('\\bpainfully\\b', 'painfully'),
    ('\\bvastly\\b', 'vastly'),
    ('\\bunbelievably\\b', 'unbelievably'),
    ('\\bway\\ too\\b', 'way too'),
    ('\\ba\\ smidge\\b', 'a smidge'),
    ('\\bfully\\b', 'fully'),
    ('\\bquantifiably\\b', 'quantifiably'),
    ('\\bdownright\\b', 'downright'),
    ('\\bsubstantially\\b', 'substantially'),
    ('\\bawfully\\b', 'awfully'),
    ('\\bjust\\ about\\b', 'just about'),
    ('\\bbarely\\b', 'barely'),
    ('\\brelatively\\b', 'relatively'),
    ('\\breal\\b', 'real'),
    ('\\bonly\\b', 'only'),
    ('\\ba\\ touch\\b', 'a touch'),
    ('\\bmoderately\\b', 'moderately'),
    ('\\breasonably\\b', 'reasonably'),
    ('\\bwholly\\b', 'wholly'),
    ('\\bridiculously\\b', 'ridiculously'),
    ('\\bstupidly\\b', 'stupidly'),
    ('\\bhugely\\b', 'hugely'),
    ('\\bfrightfully\\b', 'frightfully'),
    ('\\bunsalvageably\\b', 'unsalvageably'),
    ('\\ba\\ little\\b', 'a little'),
    ('\\bextremely\\b', 'extremely'),
    ('\\bremarkably\\b', 'remarkably'),
    ('\\bless\\ than\\b', 'less than'),
    ('\\bsort\\ of\\b', 'sort of'),
    ('\\bmarginally\\b', 'marginally'),
    ('\\bfurther\\b', 'further'),
    ('\\bjust\\b', 'just'),
    ('\\bbloody\\b', 'bloody'),
    ('\\bfundamentally\\b', 'fundamentally'),
    ('\\bhighly\\b', 'highly'),
    ('\\bcomparatively\\b', 'comparatively'),
    ('\\bdeathly\\b', 'deathly'),
    ('\\btruly\\b', 'truly'),
    ('\\bcrazily\\b', 'crazily'),
    ('\\bmeasurably\\b', 'measurably'),
    ('\\bstunningly\\b', 'stunningly'),
    ('\\bastonishingly\\b', 'astonishingly'),
    ('\\ba\\ smidgen\\b', 'a smidgen'),
    ('\\bsimply\\b', 'simply'),
    ('\\bbasically\\b', 'basically'),
    ('\\bpractically\\b', 'practically'),
    ('\\benormously\\b', 'enormously'),
    ('\\ba\\ bit\\b', 'a bit'),
    ('\\balmost\\b', 'almost'),
    ('\\ball\\ but\\b', 'all but'),
    ('\\bprofoundly\\b', 'profoundly'),
    ('\\bin\\ detail\\b', 'in detail'),
    ('\\bnoticeably\\b', 'noticeably'),
    ('\\ba\\ whisker\\b', 'a whisker'),
    ('\\bwell\\b', 'well'),
]

JP_ALL_MODIFIER_WORDS = {
    'あまり', 'あるていど', 'ある程度', 'あんがい', 'いがいに', 'いくら', 'いささか', 'いたく', 'いちじるしく', 'いちだんと', 'いちばん',
    'いっさい', 'いっそう', 'いろいろ', 'いろいろと', 'おおいに', 'おおきく', 'おおはばに', 'おおむね', 'おもいのほか', 'おもに', 'かなり',
    'かんぜんに', 'きょくりょく', 'きわめて', 'けたちがいに', 'けっこう', 'げんに', 'これいじょう', 'これ以上', 'ごく', 'さらに', 'しっかり',
    'しょうしょう', 'じゃっかん', 'じゅうぶん', 'すくなくとも', 'すこし', 'すこぶる', 'すごく', 'すべて', 'ずいぶん', 'ぜったいに', 'ぜんぜん',
    'そうとう', 'そこそこ', 'そこまで', 'それなり', 'それほど', 'そんなに', 'たいして', 'たいそう', 'たいへん', 'たくさん', 'たしょう', 'たっぷり',
    'ちょう', 'ちょっと', 'ちょっとした', 'ちょっぴり', 'ちょー', 'つよく', 'できるだけ', 'とくに', 'とっても', 'とても', 'とびきり',
    'とんでもなく', 'どうしても', 'どうも', 'なかなか', 'なにも', 'なるべく', 'はげしく', 'はなはだ', 'ばつぐんに', 'ひじょうに', 'ひどく',
    'ひろく', 'ふかく', 'べらぼうに', 'ほとんど', 'ほぼ', 'ほんとに', 'ほんの', 'まあまあ', 'まことに', 'まじ', 'ますます', 'まだまだ',
    'まったく', 'むちゃくちゃ', 'めちゃ', 'めちゃくちゃ', 'めっちゃ', 'もうすこし', 'もう少し', 'もっと', 'もっとも', 'ものすごく', 'やたら',
    'やや', 'よく', 'よっぽど', 'よほど', 'より', 'わりと', 'マジ', '一切', '一層', '一段と', '一番', '中々', '主に', '些か', '何も',
    '余り', '余程', '全く', '全て', '全然', '凄く', '出来るだけ', '割と', '十分', '厳に', '多少', '大いに', '大きく', '大して', '大変',
    '大層', '大幅に', '完全に', '少々', '少し', '少なくとも', '幾ら', '広く', '強く', '思いのほか', '意外に', '抜群に', '更に', '最も',
    '本当に', '桁違いに', '案外', '極めて', '極力', '概ね', '殆ど', '沢山', '深く', '滅茶苦茶', '激しく', '無茶苦茶', '物凄く', '特に',
    '甚だ', '痛く', '益々', '相当', '結構', '絶対に', '良く', '色々', '色々と', '若干', '著しく', '誠に', '超', '酷く', '随分',
    '非常に', '頗る', '飛び切り',
}
